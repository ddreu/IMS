    ...
    function singleBracket() {
        ...
        document.addEventListener('generate', async function() {
            ...
            try {
                ...
                if (teams.length === 2) {
                    // ✅ Handle 2-team case directly
                    generatedStructure = {
                        teams: teams,
                        matches: [{
                            match_identifier: `M${bracketManager.gameId}-D${bracketManager.departmentId}-${bracketManager.gradeLevel || "ALL"}-R1-1-${bracketManager.generateUniqueId()}`,
                            round: 1,
                            match_number: 1,
                            next_match_number: 0,
                ...
                // ✅ Use generatedStructure (not initialBracketData) to initialize
                $('#bracket-container').bracket({
                    ...
                    roundMargin: 50,
                    init: {
                        teams: generatedStructure.matches
                            .filter(m => m.round === 1)
                            .map(m => [
                                teams.find(t => t.team_id === m.teamA_id)?.team_name || null,
                                teams.find(t => t.team_id === m.teamB_id)?.team_name || null
                            ]),
                        results: Array(generatedStructure.rounds).fill([])
                    },
                    save: async function(data, userData) {
                        ...
                        generatedStructure = bracketManager.convertBracketData(data, cleanedTeams, generatedStructure.rounds);
                    },
                    decorator: {
                        edit: function(container, data, doneCb) {
                            ...
                            $('.jQBracket .label').each(function() {
                                ...
                                if (teamName && teamName !== 'BYE') selectedTeams.push(teamName);
                            });
                            teams.forEach(team => {
