...
// Validate POST data and file upload
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!isset($_POST['team_id']) || !isset($_FILES['bulk_file'])) {
        $_SESSION['error_message'] = "Invalid request.";
        header("Location: view_roster.php?team_id=" . htmlspecialchars($_POST['team_id'] ?? 0));
        exit();
    }
    ...
    if ($file['error'] !== UPLOAD_ERR_OK) {
        ...
        header("Location: view_roster.php?team_id=" . htmlspecialchars($team_id));
        exit();
    }
    ...
    try {
        ...
        foreach ($rows as $index => $data) {
            ...
            $stmtPlayers->bind_param(
                "sssii",
                $data['player_lastname'],
                $data['player_firstname'],
                $player_middlename,
                $team_id,
                $data['jersey_number']
            );
