...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $team_query = $conn->prepare("SELECT t.game_id, g.number_of_players, 
    ...
    if (substr($phone_number, 0, 1) == '0') {
        ...
        // If the number doesn't start with '+63', prepend it
    ...
    if (strlen($phone_number) != 13) {
        ...
        header("Location: player_registration.php?team_id=" . $team_id . "&grade_section_course_id=" . urlencode($grade_section_course_id));        exit();
        exit();
    }
    ...
    try {
        ...
        $stmt->execute();
        $player_id = $stmt->insert_id;  // Get the ID of the inserted player
        $stmt->close();
