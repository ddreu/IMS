...
(function () {
      ...
      xhr.onload = function () {
        ...
        try {
          ...
          if (xhr.status >= 200 && xhr.status < 300) {
            ...
            reject(
              new Error(
                `HTTP error! status: ${xhr.status}, response: ${xhr.responseText}`
              )
            );
          }
        } catch (e) {
          reject(
            new Error(
              `Parse error: ${e.message}, raw response: ${xhr.responseText}`
            )
          );
        }
      };
  ...
  window.ScoreManager.sendScoreUpdate = function () {
    ...
    const teamA_sets_won = parseInt(
      document.getElementById("teamA-sets").textContent
    );
    const teamB_sets_won = parseInt(
      document.getElementById("teamB-sets").textContent
    );
    ...
    const data = {
      ...
      timeout_teamA: parseInt(
        document.getElementById("teamA-timeouts").textContent
      ),
      timeout_teamB: parseInt(
        document.getElementById("teamB-timeouts").textContent
      ),
    };
  ...
  window.ScoreManager = {
    ...
    saveState: function () {
      ...
      localStorage.setItem(
        ...
        document.getElementById("teamA-timeouts").innerText
      );
      localStorage.setItem(
        "set_test_teamB_timeouts",
        document.getElementById("teamB-timeouts").innerText
      );
      localStorage.setItem(
    ...
    loadState: function () {
      ...
      // Restore UI elements
      if (teamATimeouts)
        document.getElementById("teamA-timeouts").innerText = teamATimeouts;
      if (teamBTimeouts)
        document.getElementById("teamB-timeouts").innerText = teamBTimeouts;
    ...
    initializeScores: function () {
      ...
      const teamATimeouts =
        localStorage.getItem("set_test_teamA_timeouts") || "0";
      const teamBTimeouts =
        localStorage.getItem("set_test_teamB_timeouts") || "0";
    ...
    syncTeamScoresIfEnabled: function () {
      ...
      const setScoreB = Math.max(0, teamB - pastTotals.teamB);

      // Format scores with leading zero
      const teamAScore = String(setScoreA).padStart(2, "0");
      const teamBScore = String(setScoreB).padStart(2, "0");

      const teamAEl = document.getElementById("scoreA");
      const teamBEl = document.getElementById("scoreB");

      if (teamAEl) teamAEl.textContent = teamAScore;
      if (teamBEl) teamBEl.textContent = teamBScore;

      // 🔁 Save and push to backend
      ScoreManager.saveState();
      ScoreManager.sendScoreUpdate();

      console.log("*✅ Synced current set scores to UI and backend:*", {
        currentSet,
        setScoreA,
        setScoreB,
      });
    ...
    endSet: function () {
      ...
      this.sendAjax("process_set_end.php", data)
        .then((response) => {
          ...
          if (response.success) {
            ...
            return this.fetchSetsWon().catch((fetchError) => {
              console.warn(
                "Sets won fetch failed, continuing with UI update",
                fetchError
              );
        ...
        .then(() => {
          ...
          window.updateScore(
            "A",
            -parseInt(document.getElementById("scoreA").textContent)
          );
          window.updateScore(
            "B",
            -parseInt(document.getElementById("scoreB").textContent)
          );
          window.updateSet(1);
      ...
      const pastScoreKey = `past_set_total_${this.scheduleId}_set${data.current_set}`;
      localStorage.setItem(
        pastScoreKey,
        JSON.stringify({
          teamA: data.teamA_score,
    ...
    endMatch: function () {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          const teamACurrentSets = parseInt(
            document.getElementById("teamA-sets").textContent
          );
          const teamBCurrentSets = parseInt(
            document.getElementById("teamB-sets").textContent
          );
          ...
          if (rawStats) {
            try {
              ...
              if (statsToSubmit.length > 0) {
                console.log(
                  "*_Submitting player stats before ending match..._*"
                );
          ...
          this.sendAjax("helper/get_bracket_type.php", {
            schedule_id: this.scheduleId,
          })
            .then((bracketData) => {
              if (!bracketData.success) {
                throw new Error(
                  bracketData.error || "Failed to get bracket type"
                );
              }
    ...
    init: function () {
      ...
      window.updateScore = function (team, change) {
        ...
        if (
          window.ScoreManager &&
          typeof ScoreManager.sendScoreUpdate === "function"
        ) {
          ScoreManager.sendScoreUpdate();
        }
      };
      ...
      // 🆕 Auto-trigger sync when syncedTeamScores changes in memory
      setInterval(() => {
        if (
          localStorage.getItem("syncPlayerStatsToScore") === "true" &&
          window.syncedTeamScores &&
          JSON.stringify(window.syncedTeamScores) !==
            window.ScoreManager._lastSyncedScores
        ) {
          window.ScoreManager._lastSyncedScores = JSON.stringify(
            window.syncedTeamScores
          );
          window.ScoreManager.syncTeamScoresIfEnabled();
        }
      }, 1000);
    },
  };
...
function applySyncToggle(enabled) {
  ...
  if (enabled && ScoreManager?.syncTeamScoresIfEnabled) {
...
function disableManualScoreButtons(disable) {
  const buttons = document.querySelectorAll(
    "#teamA .score-wrapper button, #teamB .score-wrapper button"
  );
  buttons.forEach((btn) => {
