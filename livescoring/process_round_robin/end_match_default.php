...
class RoundRobinMatchProcessor
{
    ...
    private static function updateTeamPoints($conn, $bracket_id, $team_id, $points, $is_win = null)
    {
        // Update team_tournament_points
        $points_query = "INSERT INTO team_tournament_points 
        ...
        $stmt->bind_param("iiii", $bracket_id, $team_id, $points, $points);
        $stmt->execute();
    ...
    {
        $query = "DELETE FROM live_default_scores WHERE schedule_id = ?";
        $stmt = $this->conn->prepare($query);
        $stmt->bind_param("i", $this->schedule_id);
