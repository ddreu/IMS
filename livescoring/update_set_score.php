...
try {
    ...
    // If no sets won data is provided, fetch the current sets won
    if ($teamA_sets_won === 0 && $teamB_sets_won === 0) {
        // Fetch current sets won from the database
        $sets_query = $conn->prepare("
            SELECT teamA_sets_won, teamB_sets_won 
            FROM live_set_scores 
            WHERE schedule_id = ?
        ");
        $sets_query->bind_param("i", $schedule_id);
        $sets_query->execute();
        $sets_result = $sets_query->get_result()->fetch_assoc();
        
        $teamA_sets_won = $sets_result['teamA_sets_won'];
        $teamB_sets_won = $sets_result['teamB_sets_won'];
        
        error_log("SETS WON RETRIEVED FROM DATABASE:");
        error_log("Team A Sets Won: " . $teamA_sets_won);
        error_log("Team B Sets Won: " . $teamB_sets_won);
    }
