...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.ScoreManager = {
        ...
        initializeScores: function() {
            ...
            const teamASets = localStorage.getItem('set_test_teamA_sets') || '0';
            const teamBSets = localStorage.getItem('set_test_teamB_sets') || '0';
        ...
        fetchSetsWon: function() {
            ...
            // Send request to fetch sets won
            this.sendAjax('fetch_sets_won.php', { 
                schedule_id: scheduleId,
                match_id: matchId 
            })
            .then(response => {
                console.group('Fetch Sets Won Response');
                console.log('Full Response:', response);
                console.groupEnd();

                if (response.success) {
                    // Update sets won for both teams
                    document.getElementById('teamA-sets').textContent = response.teamA_sets_won || 0;
                    document.getElementById('teamB-sets').textContent = response.teamB_sets_won || 0;
                } else {
                    console.error('Failed to fetch sets won:', response.error);
                    Swal.fire({
                        icon: 'error',
                        title: 'Fetch Error',
                        text: 'Could not retrieve sets won: ' + (response.error || 'Unknown error')
                    });
                }
            })
            .catch(error => {
                console.error('Error fetching sets won:', error);
                Swal.fire({
                    icon: 'error',
                    title: 'Network Error',
                    text: 'Could not connect to server: ' + error.message
        ...
        endSet: function() {
            ...
            const data = {
                ...
                teamA_score: parseInt(document.getElementById('scoreA').textContent),
                teamB_score: parseInt(document.getElementById('scoreB').textContent),
                teamA_sets_won: parseInt(document.getElementById('teamA-sets').textContent),
                teamB_sets_won: parseInt(document.getElementById('teamB-sets').textContent),
                current_set: parseInt(document.getElementById('currentSet').textContent),
                timeout_teamA: parseInt(document.getElementById('teamA-timeouts').textContent),
                timeout_teamB: parseInt(document.getElementById('teamB-timeouts').textContent)
            };
                ...
                .then(response => {
                    ...
                    if (response.success) {
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'Error',
                            text: response.error || 'Failed to end set'
                        });
                    }
                })
                .catch(error => {
