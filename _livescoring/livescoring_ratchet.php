...
<head>
    ...
    <style>
        ...
        .floating-preview .control-buttons {
            position: absolute;
            top: -10px;
            right: -10px;
            display: flex;
        ...
        .floating-preview .stop-stream {
            background: #dc3545;
        }
        .floating-preview .minimize-stream {
            background: #0d6efd;
        }
    </style>
</head>
...
<body>
    ...
    <div class="main-container">
        <div class="main-content">
            ...
            <div class="position-fixed top-0 start-0 m-3">
                <button class="btn btn-success rounded-circle" type="button" onclick="openLiveStreamSettings()">
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <div class="stream-options">
                                    <div class="mb-3">
                                        <button class="btn btn-lg btn-outline-primary w-100" onclick="startStream()">
                                            <i class="fas fa-webcam me-2"></i>Use Computer Webcam
                                        </button>
                                    </div>
                                    <div class="mb-3">
                                        <button class="btn btn-lg btn-outline-primary w-100" onclick="startMobileStream()">
                                            <i class="fas fa-mobile-alt me-2"></i>Use Phone Camera
                                        </button>
                                    </div>
                                    <div class="mb-3">
                                        <button class="btn btn-lg btn-outline-primary w-100" onclick="startScreenShare()">
                                            <i class="fas fa-desktop me-2"></i>Share Screen
                                        </button>
                                    </div>
                                </div>
                                <div class="preview-container mt-3">
                                    <video id="videoPreview" class="w-100" autoplay muted playsinline style="background: #000; min-height: 360px;"></video>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <div class="floating-preview" id="floatingPreview">
        <div class="control-buttons">
            <button class="control-button minimize-stream" onclick="toggleMinimize()" title="Minimize/Maximize">
                <i class="fas fa-compress-alt"></i>
            </button>
            <button class="control-button stop-stream" onclick="stopStream()" title="Stop Stream">
    ...
    <script>
        ...
        // Function to open stream settings modal
        function openLiveStreamSettings() {
            const modal = new bootstrap.Modal(document.getElementById('liveStreamSettingsModal'));
            modal.show();
        }
        ...
        function toggleMinimize() {
            ...
            const button = preview.querySelector('.minimize-stream i');
        ...
        // Function to show floating preview
        function showFloatingPreview(stream) {
            const modal = bootstrap.Modal.getInstance(document.getElementById('liveStreamSettingsModal'));
            if (modal) {
                modal.hide();
            }
        ...
        // Start streaming with webcam
        async function startStream() {
            try {
                // Stop any existing stream
                stopStream();
                
                console.log('Starting stream for schedule ID:', scheduleId);
                ...
                console.log('MediaRecorder created with settings:', options.videoBitsPerSecond, options.mimeType);
                ...
                ws.onopen = () => {
                    ...
                    mediaRecorder.ondataavailable = async (event) => {
                        if (event.data.size > 0 && ws && ws.readyState === WebSocket.OPEN) {
                            chunkCounter++;
                            console.log(`Processing chunk ${chunkCounter}, size: ${event.data.size} bytes`);
                    ...
                    // Set chunk interval (250ms for more frequent updates)
        ...
        // Start streaming with mobile camera
        async function startMobileStream() {
            try {
                ...
                alert('Error starting mobile stream: ' + error.message);
                stopStream();
            }
        }
        ...
        async function startScreenShare() {
            try {
                ...
                console.log('Starting screen share for schedule ID:', scheduleId);
                ...
                const options = {
                    mimeType: 'video/webm;codecs=vp8',
                    videoBitsPerSecond: 2500000 // 2.5 Mbps for screen sharing
                };
                mediaRecorder = new MediaRecorder(stream, options);
                console.log('MediaRecorder created with settings:', options.videoBitsPerSecond, options.mimeType);
        ...
        document.getElementById('liveStreamSettingsModal').addEventListener('hidden.bs.modal', function () {
            stopStream();
        });
    </script>