...
function startTimer() {
 ...
 function initPlayerStatsToggle() {
     ...
     window.togglePlayerStats = function() {
         ...
         const panel = document.querySelector('.player-stats') || document.getElementById('playerStatsPanel');
         const overlayEl = document.querySelector('.overlay') || document.getElementById('playerStatsOverlay');
     ...
     function addEventListeners() {
         const statsBar = document.querySelector('.player-stats-bar') || document.getElementById('playerStatsBar');
         const mobileButton = document.querySelector('.bottom-nav .nav-item:last-child') || document.getElementById('mobilePlayerStatsButton');
         const overlayEl = document.querySelector('.overlay') || document.getElementById('playerStatsOverlay');
     ...
     function retryInitialization(maxAttempts = 5) {
         ...
         const interval = setInterval(() => {
             ...
             if (attempts >= maxAttempts) {
                 clearInterval(interval);
                 console.error('Failed to initialize player stats after multiple attempts');
                 findElementsDebug();
             }
         }, 500);
     }
 ...
 function findElementsDebug() {
     ...
     console.log('Full document body structure:', 
 ...
 const playerStatsManager = {
     init: function() {
         ...
         document.addEventListener('click', function(event) {
             if (document.querySelector('.player-stats') && document.querySelector('.player-stats').classList.contains('active') &&
...
function updateShotClock() {
    document.getElementById("shotClock").textContent = shotClockTime.toString().padStart(2, '0');
}
...
function sendUpdate() {
    ...
    console.log('Game IDs from gameData:', { schedule_id, game_id, teamA_id, teamB_id });
    ...
    const teamAScore = parseInt(document.getElementById('teamAScore').textContent) || 0;
    const teamBScore = parseInt(document.getElementById('teamBScore').textContent) || 0;
    const foulTeamA = parseInt(document.getElementById('teamAFouls').textContent) || 0;
    const foulTeamB = parseInt(document.getElementById('teamBFouls').textContent) || 0;
    const timeoutTeamA = parseInt(document.getElementById('teamATimeouts').textContent) || 0;
    const timeoutTeamB = parseInt(document.getElementById('teamBTimeouts').textContent) || 0;
    const currentPeriod = parseInt(document.getElementById('periodCounter').textContent) || 1;
...
const stateManager = {
    ...
    restore: function() {
        ...
        if (savedState) {
            ...
            // Restore timeouts
            document.getElementById('teamATimeouts').textContent = state.teamATimeouts;
            document.getElementById('teamBTimeouts').textContent = state.teamBTimeouts;
            
            // Restore period
            document.getElementById('periodCounter').textContent = state.periodCounter;
