class DoubleBracketManager {
  ...
  handleBracketUpdate(data) {
    ...
    results[0].forEach((round, roundIndex) => {
      round.forEach((matchResult, matchIndex) => {
        const matchNumber =
          matchIndex + 1 + roundIndex * Math.pow(2, roundIndex);
        ...
        if (match) {
          ...
          match.status =
            matchResult[0] !== null || matchResult[1] !== null
              ? "Finished"
              : "Pending";
        }
      });
      ...
      round.forEach((matchResult, matchIndex) => {
        const match = this.matches.find(
          (m) =>
            ...
            m.match_number ===
              matchIndex +
                this.matches.filter((m) => m.bracket === "winners").length +
                1
        );
    ...
    if (results[2] && results[2][0] && results[2][0][0]) {
      ...
      if (finalMatch) {
        ...
        finalMatch.status =
          results[2][0][0][0] !== null || results[2][0][0][1] !== null
            ? "Finished"
            : "Pending";
      }
    }
  ...
  handleTeamEdit(container, data, doneCb) {
    ...
    input.on("change", (event) => {
      ...
      const selectedTeam =
        selectedValue === "BYE" || selectedValue === "" ? null : selectedValue;

      // Update the teams array
      doneCb(selectedTeam);

      // Update the matches array
      this.updateMatchesWithNewTeam(selectedTeam, data);
    });
  }
  ...
  validateTeamCount(teamCount) {
    if (teamCount < this.MIN_TEAMS) {
      throw new Error(
        `At least ${this.MIN_TEAMS} teams are required for a tournament`
      );
    }
  ...
  convertBracketData(bracketData, teams, totalRounds) {
    ...
    bracketData.winners_bracket.forEach((match) => {
      ...
      if (
        doubleEliminationData.results[0][roundIndex] &&
    ...
    bracketData.losers_bracket.forEach((match) => {
      ...
      const matchIndex = match.match_number - 1;
      if (
        doubleEliminationData.results[1][roundIndex] &&
  ...
  generateDoubleBracketStructure() {
    ...
    // Generate matches
    const matches = this.generateAllMatches(
      teamPairs,
      winnersRounds,
      losersRounds
    );
    ...
    for (let i = 0; i < losersRounds; i++) {
      bracketData.results[1].push(
        new Array(Math.pow(2, Math.floor((losersRounds - i - 1) / 2))).fill([
          null,
          null,
        ])
      );
  ...
  generateAllMatches(teamPairs, winnersRounds, losersRounds) {
      ...
      for (let i = 0; i < matchesInRound; i++) {
        if (round === 1) {
          ...
          const teamAId =
            teamA === "BYE"
              ? -1
              : this.teams.find((t) => t.team_name === teamA)?.team_id || -2;
          const teamBId =
            teamB === "BYE"
              ? -1
              : this.teams.find((t) => t.team_name === teamB)?.team_id || -2;

          matches.push({
            match_identifier: `M${this.gameId}-D${
              this.departmentId
            }-W-R${round}-${matchNumber}-${Date.now()}-${this.generateUniqueId()}`,
            round: round,
            match_number: matchNumber,
            bracket: "winners",
            next_winner_match: this.calculateNextWinnerMatch(
              round,
              matchNumber
            ),
            ...
            match_type:
              round === winnersRounds ? "winners_final" : "winners_regular",
          ...
          matches.push({
            ...
            next_winner_match:
              round < winnersRounds
                ? this.calculateNextWinnerMatch(round, matchNumber)
                : 0,
            next_loser_match: this.calculateNextLoserMatch(round, matchNumber),
            teamA_id: -2,
            teamB_id: -2,
    ...
    for (let round = 1; round <= losersRounds; round++) {
      const matchesInRound = Math.pow(
        2,
        Math.floor((losersRounds - round) / 2)
      );
      ...
      for (let i = 0; i < matchesInRound; i++) {
        matches.push({
          match_identifier: `M${this.gameId}-D${
            this.departmentId
          }-L-R${round}-${matchNumber}-${Date.now()}-${this.generateUniqueId()}`,
          round: round,
          match_number: matchNumber,
          bracket: "losers",
          next_winner_match:
            round < losersRounds ? matchNumber + matchesInRound : 14, // Finals match
          next_loser_match: 0,
          teamA_id: -2,
          teamB_id: -2,
          status: "Pending",
          match_type:
            round === losersRounds ? "losers_final" : "losers_regular",
          // score_teamA: null,
          // score_teamB: null
        });
        matchNumber++;
    ...
    matches.push({
      match_identifier: `M${this.gameId}-D${
        this.departmentId
      }-F-R1-${matchNumber}-${Date.now()}-${this.generateUniqueId()}`,
      round: 1,
      match_number: matchNumber,
      bracket: "finals",
      next_winner_match: 0,
  ...
  formatLosersResults() {
    ...
    for (let round = 0; round < loserRounds; round++) {
      ...
      const currentRoundMatches =
        round % 2 === 0 ? matchesInRound : matchesInRound;
          ...
          switch (match.bracket) {
            case "winners":
              if (currentData.results[0][match.round - 1]) {
                currentData.results[0][match.round - 1][
            ...
            case "losers":
              if (currentData.results[1][match.round - 1]) {
                currentData.results[1][match.round - 1][
                  match.match_number - 1
                ] = scores;
              }
              break;
  ...
  handleByeMatch(match) {
    ...
    if (match.teamA_id === -1 || match.teamB_id === -1) {
      ...
      const winningTeamId =
        match.teamA_id === -1 ? match.teamB_id : match.teamA_id;
      ...
      if (match.next_winner_match) {
        const nextMatch = this.matches.find(
          (m) => m.match_number === match.next_winner_match
        );
        if (nextMatch) {
  ...
  processByeMatches() {
    ...
    const firstRoundMatches = this.matches.filter(
  ...
  updateMatchTeam(matchIdentifier, teamA, teamB) {
    const match = this.matches.find(
      (m) => m.match_identifier === matchIdentifier
    );
    if (match) {
      ...
      console.log(
        `Updated match ${matchIdentifier} with teams: ${teamA}, ${teamB}`
      );
    }
  }

  updateMatchesWithNewTeam(newTeam, oldTeam) {
    const newTeamId = this.getTeamId(newTeam);
    const oldTeamId = this.getTeamId(oldTeam);

    this.matches.forEach((match) => {
      if (match.teamA_id === oldTeamId) {
        match.teamA_id = newTeamId;
      }
      if (match.teamB_id === oldTeamId) {
        match.teamB_id = newTeamId;
      }
    });
  ...
  getTeamId(teamName) {
    if (teamName === "BYE") return -1;
    const team = this.teams.find((t) => t.team_name === teamName);
