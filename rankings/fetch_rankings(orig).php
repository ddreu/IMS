...
if ($game_id) {
    ...
    while ($bracket = $brackets->fetch_assoc()) {
        ...
        // Check bracket type
        $bracketTypeQuery = "SELECT bracket_type FROM brackets WHERE bracket_id = ?";
        $btStmt = $conn->prepare($bracketTypeQuery);
        $btStmt->bind_param('i', $bracketId);
        $btStmt->execute();
        $btResult = $btStmt->get_result()->fetch_assoc();
        $bracketType = $btResult['bracket_type'];

        if ($bracketType === 'round_robin') {
            // Use team_tournament_points for round robin
            $teamQuery = "
        SELECT t.grade_section_course_id, p.total_points
        FROM team_tournament_points p
        ...
        LIMIT 3";
            ...
            if ($bracketGradeLevel) {
                ...
                $stmt->bind_param('iii', $bracketId, $department_id, $bracketGradeLevel);
            } else {
                $stmt = $conn->prepare($teamQuery);
                $stmt->bind_param('ii', $bracketId, $department_id);
            }
        } else {
            // Use team wins for other types
            $teamQuery = "
        SELECT t.grade_section_course_id, t.wins
        FROM teams t
        JOIN grade_section_course gsc ON t.grade_section_course_id = gsc.id
        WHERE t.game_id = ?
          AND gsc.department_id = ?
